...
class TestLibraryRoutes:
    async def test_get_books(self, ac: AsyncClient, session: AsyncSession):
        ...
        assert book_data == response.json()
    ...
    async def test_get_books_with_title_filter(
        ...
        response = await ac.get(url="/books/", params={"title": "testtitle"})
    ...
    async def test_create_book(
        ...
        assert book._to_dict() == response.json()
    ...
    async def test_delete_book(
        ...
        assert response.json() == {"Result": "Successfully deleted"}
